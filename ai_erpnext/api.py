...
# def _documents_for_action(data, action=None):
#     """Return one or multiple extracted documents from the AI response."""
#     documents = data.get("documents") or []
...
# @frappe.whitelist()
# def create_from_extracted(extracted_data_json, action="bom_only"):
#     """Create Bill of Materials documents from extracted AI data."""
#     try:
#         data = (
...
#             frappe.throw("Invalid extracted document data")
...
#             # This API is now dedicated to BOM creation.
#             document_data["document_type"] = "Bill of Materials"

#             if not document_data.get("item") and not document_data.get("item_name"):
#                 frappe.throw("Finished item is missing in BOM data")

#             if not document_data.get("items"):
#                 frappe.throw("No component items found for the BOM")

#             doctype, name = create_document(document_data)
...
#         if not results:
#             frappe.throw("No valid BOM documents were found to create")
...
from ai_erpnext.erpnext_mapper import (
    create_bom,
    create_items_from_master,
)


# Only BOM-related actions now. "bom_only" is used for the parent+component
# flow (Review modal in the inbox / upload page). "items_only" is available
# for flat master sheets (like Tools_Master_Sheet.xlsx) where every row is
# an independent Item with no parent/component relationship.
BOM_ACTIONS = {"bom_only", "items_only"}


def _documents_for_action(data, action):
    """
    Some raw extractions return multiple BOM documents in one file
    (data["documents"] = [ {...bom1...}, {...bom2...} ]).

    IMPORTANT: once a document has been through the Review modal, the
    top-level payload itself IS the single, user-edited BOM (it has its
    own "item"/"items"). In that case we must use it directly and ignore
    any stale "documents" array left over from the original extraction —
    otherwise edits made in the review screen (like typing in the Parent
    Item) get silently discarded and you see "Parent/Finished Item missing".
    """
    has_own_parent_item = bool(
        data.get("item") or data.get("item_name")
        or data.get("parent_item") or data.get("finished_item")
    )
    if action in BOM_ACTIONS and not has_own_parent_item:
        documents = data.get("documents") or []
        if documents:
            return documents
    return [data]
...
def process_document(file_url):
    try:
        ...
        if not extracted.get("items") or len(extracted["items"]) == 0:
            return {
                "success": False,
                "error": "No component/material rows found in document. Is this a BOM or material sheet?",
                "stage": "extraction",
                "raw_extracted": extracted
            }
        ...
        return {
            ...
            "stage": "extracted",
            "extracted_data": extracted,
            "suggested_doctype": extracted.get("document_type", "Bill of Materials")
        }
...
def create_from_extracted(extracted_data_json, action):
    try:
        ...
        for document_data in documents:
            if action == "bom_only":
                name = create_bom(document_data)
                results.append({"doctype": "BOM", "name": name})

            elif action == "items_only":
                summary = create_items_from_master(document_data)
                results.append({"doctype": "Item", "name": summary})

            else:
                frappe.throw(f"Unsupported action '{action}'. Only Bill of Materials creation is supported.")
...
def get_email_queue(
    ...
    # Only one document type is created now: Bill of Materials.
    business_doctypes = [
        "Bill of Materials",
        "BOM",
    ]
