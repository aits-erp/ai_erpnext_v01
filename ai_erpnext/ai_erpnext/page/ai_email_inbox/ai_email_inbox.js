        ...
        <div id="inbox-modal-overlay" class="modal-overlay" style="display:none">
            <div class="inbox-modal">
                ...
                <div class="inbox-modal-body">
                    ...
                    <div class="imodal-tab-content active" id="imodal-extracted">
                        <div class="ai-summary-grid" id="imodal-summary"></div>
                        <div class="section-label" style="margin-top:14px">Component Items</div>
                        <div class="ai-table-wrap">
                            <table class="ai-table">
                                <thead>
                                    <tr>
                                        <th>COMPANY_CODE</th>
                                        <th>MATERIAL_USAGE_CODE</th>
                                        <th>MATERIAL_GROUP</th>
                                        <th>MATERIAL_SUBGROUP</th>
                                        <th>MATERIAL_SUBSUBGROUP</th>
                                        <th>PURCHASE_UOM_CODE</th>
                                        <th>STOCKING_UOM_CODE</th>
...
var _search_timer   = null;
var _sync_in_progress = false;
var _default_company  = null;

function get_default_company() {
    // Cache the site's default Company so every row can be pre-filled
    // with COMPANY_CODE without a round-trip per row.
    if (_default_company !== null) return _default_company;
    try {
        _default_company = frappe.defaults.get_default('company') || '';
    } catch (e) {
        _default_company = '';
    }
...
function open_inbox_modal(queue_name) {
    frappe.call({
        ...
        callback: function(r) {
            ...
            var ext = d.extracted || {};
            var parent_item = ext.item || ext.item_name || '';
            var default_company = get_default_company();

            // Summary: Type is fixed to Bill of Materials, Parent Item / Qty / UOM editable
            $('#imodal-summary').html(`
                <div class="ai-sum-card">
                    <div class="ai-sum-label">Type</div>
                    <div class="ai-sum-val">🧩 Bill of Materials</div>
                </div>
                <div class="ai-sum-card">
                    <div class="ai-sum-label">Parent Item</div>
                    <input class="review-input" id="review-parent-item" value="${escape_attr(parent_item)}" placeholder="Enter parent/finished item name">
                </div>
                <div class="ai-sum-card">
                    <div class="ai-sum-label">Quantity</div>
                    <input class="review-input" id="review-quantity" type="number" step="any" value="${escape_attr(ext.quantity || 1)}">
                </div>
                <div class="ai-sum-card">
                    <div class="ai-sum-label">UOM</div>
                    <input class="review-input" id="review-uom" value="${escape_attr(ext.uom || 'Nos')}">
                </div>
            `);

            var rows = (ext.items || []).map(function(i, idx) {
                var stock_uom = i.uom || i.stocking_uom_code || 'Nos';
                var purchase_uom = i.purchase_uom || i.purchase_uom_code || '';
                var conv_factor = i.conversion_factor || 1;
                var company_code = i.company_code || default_company;
                var usage_code = i.material_usage_code || '';
                return `<tr>
                    <td><input class="review-input review-item-company" data-idx="${idx}" value="${escape_attr(company_code)}" placeholder="Company code"></td>
                    <td><input class="review-input review-item-usage" data-idx="${idx}" value="${escape_attr(usage_code)}" placeholder="Usage code"></td>
                    <td><input class="review-input review-item-group" data-idx="${idx}" value="${escape_attr(i.material_group || '')}" placeholder="Group"></td>
                    <td><input class="review-input review-item-subgroup" data-idx="${idx}" value="${escape_attr(i.material_subgroup || '')}" placeholder="Subgroup"></td>
                    <td><input class="review-input review-item-subsubgroup" data-idx="${idx}" value="${escape_attr(i.material_subsubgroup || '')}" placeholder="Sub-subgroup"></td>
                    <td><input class="review-input review-item-purchase-uom" data-idx="${idx}" value="${escape_attr(purchase_uom)}" placeholder="Purchase UOM"></td>
                    <td><input class="review-input review-item-stock-uom" data-idx="${idx}" value="${escape_attr(stock_uom)}" placeholder="Stock UOM"></td>
                    <td><input class="review-input review-item-conv" data-idx="${idx}" type="number" step="any" value="${escape_attr(conv_factor)}"></td>
                    <td><input class="review-input review-item-name" data-idx="${idx}" value="${escape_attr(i.item_name || '')}" placeholder="Material description"></td>
                    <td><input class="review-input review-item-hsn" data-idx="${idx}" value="${escape_attr(i.hsn_code || '')}" placeholder="Optional"></td>
                    <td><input class="review-input review-item-qty" data-idx="${idx}" type="number" step="any" value="${escape_attr(i.qty || 1)}"></td>
                </tr>`;
            }).join('');
            $('#imodal-items').html(rows || '<tr><td colspan="11" style="text-align:center;color:#aaa">No components. Re-extract or add rows in the source file.</td></tr>');
            ...
            // Single action: Create BOM
            var btn_html = `<button class="action-choice-btn" style="border-left:4px solid #7c6fcd" onclick="inbox_create('bom_only','${queue_name}')">🧩 Create Bill of Materials</button>`;
            ...
            if (!has_items) {
                $('#imodal-actions').prepend(`
                    <div style="width:100%;margin-bottom:12px;padding:10px;background:#fff8e1;border-radius:6px;border-left:4px solid #ffc107;">
                        <div style="font-size:12px;color:#666;margin-bottom:8px;">
                            ⚠️ No components extracted. Re-extract with updated AI prompt:
                        </div>
                    </div>
                `);
            }
...
function collect_review_data() {
    ...
    var parent_item = ($('#review-parent-item').val() || '').trim();
    if (!parent_item) {
        frappe.msgprint('Please enter the Parent/Finished Item before creating the BOM.');
        return null;
    }

    ext.document_type = 'Bill of Materials';
    ext.item = parent_item;
    ext.item_name = parent_item;
    ext.quantity = flt($('#review-quantity').val()) || 1;
    ext.uom = ($('#review-uom').val() || 'Nos').trim();

    // Drop any stale multi-document array from the original raw extraction —
    // this reviewed object IS the single BOM now, and _documents_for_action()
    // in api.py must use it directly rather than falling back to old sub-docs.
    delete ext.documents;
    ...
    $('#imodal-items tr').each(function() {
        ...
        ext.items.push({
            item_code: original_item.item_code || '',
            item_name: item_name,
            company_code: ($(this).find('.review-item-company').val() || '').trim(),
            material_usage_code: ($(this).find('.review-item-usage').val() || '').trim(),
            material_group: ($(this).find('.review-item-group').val() || '').trim(),
            material_subgroup: ($(this).find('.review-item-subgroup').val() || '').trim(),
            material_subsubgroup: ($(this).find('.review-item-subsubgroup').val() || '').trim(),
            qty: qty,
            uom: ($(this).find('.review-item-stock-uom').val() || 'Nos').trim(),
            purchase_uom: ($(this).find('.review-item-purchase-uom').val() || '').trim(),
            conversion_factor: flt($(this).find('.review-item-conv').val()) || 1,
            hsn_code: ($(this).find('.review-item-hsn').val() || '').trim()
        });
    });

    if (!ext.items.length) {
        frappe.msgprint('Please add at least one component before creating the BOM.');
        return null;
    }
...
function inject_inbox_styles() {
    $('<style>').text(`
        ...
        /* Modal */
        .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.55); z-index: 9999; display: flex; align-items: center; justify-content: center; padding: 20px; }
        .inbox-modal   { background: #fff; border-radius: 14px; width: 100%; max-width: 900px; max-height: 90vh; overflow-y: auto; box-shadow: 0 24px 80px rgba(0,0,0,0.25); }
        .inbox-modal-header { display: flex; justify-content: space-between; align-items: flex-start; padding: 18px 20px; border-bottom: 1px solid #eee; position: sticky; top: 0; background: #fff; z-index: 1; border-radius: 14px 14px 0 0; }
        .imodal-subject { font-weight: 700; font-size: 15px; color: #1a1a2e; }
        .imodal-meta    { font-size: 12px; color: #aaa; margin-top: 4px; }
        .modal-close    { background: #f5f5f5; border: none; width: 28px; height: 28px; border-radius: 50%; font-size: 16px; cursor: pointer; color: #888; display: flex; align-items: center; justify-content: center; transition: background 0.15s; }
        ...
        .ai-table-wrap { overflow-x: auto; border: 1px solid #f0f0f0; border-radius: 8px; margin-top: 8px; }
        .ai-table    { width: 100%; border-collapse: collapse; font-size: 13px; }
        .ai-table th { background: #f8f8f8; padding: 8px 10px; text-align: left; font-weight: 700; color: #666; font-size: 12px; white-space: nowrap; }
        .ai-table td { padding: 8px 10px; border-bottom: 1px solid #f5f5f5; }
        .ai-table tr:last-child td { border-bottom: none; }
        ...
        .review-item-name { min-width: 200px; }
        .review-item-company { min-width: 90px; }
        .review-item-usage { min-width: 90px; }
        .review-item-group { min-width: 90px; }
        .review-item-subgroup { min-width: 110px; }
        .review-item-subsubgroup { min-width: 110px; }
        .review-item-qty { min-width: 70px; }
        .review-item-stock-uom { min-width: 80px; }
        .review-item-purchase-uom { min-width: 90px; }
        .review-item-conv { min-width: 80px; }
        .review-item-hsn { min-width: 90px; }
