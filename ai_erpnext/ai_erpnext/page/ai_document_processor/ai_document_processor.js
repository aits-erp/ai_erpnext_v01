    ...
    $(wrapper).find('.layout-main-section').html(`
        <div class="ai-root">
            ...
            <div class="ai-tab-content active" id="tab-upload">
                <div class="ai-card">
                    <div class="ai-drop-zone" id="ai-drop-zone">
                        <div class="ai-drop-icon">🧩</div>
                        <div class="ai-drop-title">Drop file here or click to browse</div>
                        <div class="ai-drop-sub">XLSX, CSV, PDF, JPG, PNG — Tools / Material Master Sheet</div>
                        <input type="file" id="ai-file-input" accept=".pdf,.jpg,.jpeg,.png,.webp,.xlsx,.xls,.csv" style="display:none">
                    </div>
                    <div id="ai-file-info" style="display:none" class="ai-file-info">
                        <span id="ai-file-icon">📎</span>
                        <span id="ai-file-name-text"></span>
                ...
                <div id="ai-action-panel" style="display:none" class="ai-card">
                    ...
                    <!-- Items Table -->
                    <div class="ai-section-title" style="margin-top:16px">🧩 Component Items</div>
                    <div class="ai-table-wrap">
                        <table class="ai-table" id="ai-items-table">
                            <thead>
                                <tr>
                    ...
                    <!-- Action Choices -->
                    <div class="ai-section-title" style="margin-top:20px">⚡ Create Document</div>
                    <div class="ai-action-grid" id="ai-action-grid"></div>
            ...
            <div class="ai-tab-content" id="tab-emails">
                ...
                <div id="email-queue-empty" style="display:none" class="ai-empty-state">
                    <div style="font-size:40px">📭</div>
                    <div>No pending emails with tools/material sheets</div>
                    <div style="font-size:12px;color:#aaa;margin-top:6px">
                        Queue updates automatically when new emails arrive
                    </div>
                </div>
        ...
        <div id="ai-modal-overlay" class="ai-modal-overlay" style="display:none">
            <div class="ai-modal">
                ...
                <div class="ai-modal-body">
                    <div id="modal-summary-grid" class="ai-summary-grid"></div>
                    <div class="ai-section-title" style="margin-top:12px">Component Items</div>
                    <div class="ai-table-wrap">
                        <table class="ai-table">
                            <thead><tr><th>Item</th><th>Qty</th><th>UOM</th><th>HSN Code</th></tr></thead>
                            <tbody id="modal-items-body"></tbody>
                        </table>
                    </div>
                    <div class="ai-section-title" style="margin-top:16px">Create As:</div>
...
// ── FILE UPLOAD ──────────────────────────────────────────
var _uploaded_url = null;
var SUPPORTED_EXTENSIONS = ['pdf', 'jpg', 'jpeg', 'png', 'webp', 'xlsx', 'xls', 'csv'];
...
function handle_file(file) {
    ...
    $('#ai-file-info').show();
    $('#ai-file-name-text').text(file.name);
    $('#ai-file-icon').text(['xlsx', 'xls', 'csv'].includes(ext) ? '📊' : (ext === 'pdf' ? '📄' : '🖼️'));
    $('#ai-extract-btn').hide();
    frappe.show_alert({message: 'Uploading...', indicator: 'blue'});
...
// ── RENDER EXTRACTED DATA + ACTION BUTTON ────────────────
function render_action_panel(data, summaryEl, itemsEl, actionsEl, context) {

    // Summary cards — BOM shape: parent item, quantity, uom, component count
    var parent_item = data.item || data.item_name || '—';
    var qty         = data.quantity || 1;
    var uom         = data.uom || 'Nos';
    var comp_count  = (data.items || []).length;

    $(summaryEl).html(`
        <div class="ai-sum-card"><div class="ai-sum-label">Document Type</div><div class="ai-sum-val">Bill of Materials</div></div>
        <div class="ai-sum-card"><div class="ai-sum-label">Parent Item</div><div class="ai-sum-val">${parent_item}</div></div>
        <div class="ai-sum-card"><div class="ai-sum-label">Quantity / UOM</div><div class="ai-sum-val">${qty} ${uom}</div></div>
        <div class="ai-sum-card"><div class="ai-sum-label">Components</div><div class="ai-sum-val">${comp_count}</div></div>
    `);

    // Items table — components: item_name, qty, uom, hsn_code
    var rows = '';
    (data.items || []).forEach(function(item) {
        rows += `<tr>
            <td>${item.item_name || '—'}</td>
    ...
    $(itemsEl).html(rows || '<tr><td colspan="4" style="text-align:center;color:#aaa">No components found</td></tr>');

    // Single action: Create BOM
    var btns = `<button class="ai-action-btn"
                    style="border-left: 4px solid #7c6fcd"
                    data-action="bom_only"
                    data-context="${context}">
    ...
    frappe.call({
        ...
        callback: function(r) {
            ...
            if (r.message && r.message.success) {
                ...
                frappe.show_alert({message: 'BOM created successfully!', indicator: 'green'});
            } else {
                var err = (r.message && r.message.error) || 'Creation failed';
                frappe.show_alert({message: err, indicator: 'red'});
                if (context === 'upload') {
